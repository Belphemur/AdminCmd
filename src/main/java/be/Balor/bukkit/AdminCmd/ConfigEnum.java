...
public enum ConfigEnum {
	...
	H_C_NORMAL("help.color.normal", "f", "Number (hexa decimal) representing the normal color of the help text."),
	H_C_REQ_PARAM("help.color.required", "c", "Number (hexa decimal) representing the color of the required params in the help text."),
	H_C_OPT_PARAM("help.color.optional", 2, "Number (hexa decimal) representing the color of the optional params in the help text."),
	H_C_CMD("help.color.command", 6, "Number (hexa decimal) representing the color of the requiered params in the help text."),
	...
	ADD_BANNER_IN_BAN("addBannerNameInBan", true, "Add or not the 'by Player' in the ban reason"),
	CHECK_UPDATE("checkForUpdate.status", true, "If AdminCmd has to check if there is a new version of the plugin."),
	UPDATE_SRC("checkForUpdate.updateChannel", "stable", "Which version to check. Can be stable or dev."),
	E_PST_DELAY("delayBeforeSendToDb", 10, "Number of seconds before the plugin execute the request to the database (in case of SQLite or MySQL).\n"
			+ "This feature have been done to avoid lag by execut the request in another thread than the main one."),
	MK_V_G_KILL("mk.KillVillagerAndGolem", false, "If set to true, the mobkill command when used with ALL will also kill the vilager and the golem."),
	MK_DEF_RADIUS("mk.DefaultRadiusInBlock", 5, "Set the default radius for the mobkill command."),
	...
	PLUGIN_HIDE(
        ...
        USE_UNKNOWN_COMNAD("unknownCommand.enable", Boolean.valueOf(true), "Enable the change of the default unknown command message."),
        UNKNOWN_COMMAND_BYPASS("unknownCommand.bypassCommands", Arrays.asList(new String[]{"example", "test"}), "Add here commands that should not be checked from the unknownCommand feature (Case sensitive). Usefull for plugins like MakeYourOwnCommands, MCDocs or MyCommand.");
	...
	private static String getHeader() {
		...
		for (final ConfigEnum ce : values()) {
			buffer.append(ce.confVal).append("\t:\t").append(ce.description).append(" (Default : ").append(ce.defaultVal).append(')').append('\n');
		}
