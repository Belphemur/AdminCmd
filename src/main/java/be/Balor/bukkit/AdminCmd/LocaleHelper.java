...
public enum LocaleHelper {
	ITEMID("itemId", ChatColor.GOLD + "%player" + ChatColor.GRAY
			+ " is holding " + ChatColor.GREEN + "%item" + ChatColor.WHITE
			+ ":" + ChatColor.RED + "%data"),
	EGG_PARAM("eggCmdParam", ChatColor.RED + "The parameter " + ChatColor.GOLD
			+ "-t" + ChatColor.RED + " is needed to choose the egg."),
	EGG_PARAM_BLOCK("eggBlockParam", ChatColor.RED + "The parameter "
			+ ChatColor.GOLD + "-b" + ChatColor.RED
			+ " is needed to choose the block to use for replacement."),
	EGG_PARAM_ENTITY("eggEntityParam", ChatColor.RED + "The parameter "
			+ ChatColor.GOLD + "-e" + ChatColor.RED
			+ " is needed to choose the entity id to spawn."),
	EGG_PARAM_MOB("eggMobParam", ChatColor.RED + "The parameter "
			+ ChatColor.GOLD + "-m" + ChatColor.RED
			+ " is needed to choose the mob to spawn."),
	DONT_EXISTS("dontExists", ChatColor.RED + "The %type " + ChatColor.GOLD
			+ "%value " + ChatColor.RED + "don't exists !"),
	...
	POTION_EFFECT("potionEffect", ChatColor.MAGIC + "magic" + ChatColor.AQUA
			+ "%player" + ChatColor.LIGHT_PURPLE + " had drink the potion"
			+ ChatColor.GOLD + " %potion !" + ChatColor.MAGIC + "magic"),
	SPYMSG_TITLE("spymsgTitle", ChatColor.GREEN + "[SpyMsg]" + ChatColor.WHITE),
	SPYMSG_HEADER("spymsgMessageHeader", "#spymsgTitle# " + "%sender" + "-"
	...
	PLAYER_KICKED("playerKicked", ChatColor.GOLD + "%player "
			+ ChatColor.YELLOW + "has been kicked from the Server, reason:"
			+ ChatColor.RED + " %reason"),
	PLAYER_QUITCMD_MSG("quitCmdMsg", ChatColor.GOLD + "%player "
			+ ChatColor.YELLOW + "left the game: %reason"),
	WORLD_NOT_LOADED("worldNotLoaded", ChatColor.RED
			+ "This World is not loaded :" + ChatColor.GOLD + " %message"),
	MISSING_ARG("missingArg", ChatColor.RED
			+ "You must provide a %arg as an argument to use the" + " command "
			+ ChatColor.AQUA + "%cmdName"),
	...
	TP_DIM_LIST("dimTpList", ChatColor.GREEN
			+ "Worlds which are currently available to teleport to: " + " //n "
			+ ChatColor.GOLD + "%list"),
	TP_ALL("tpAll", ChatColor.GREEN
			+ "You have been successfully teleported to " + ChatColor.GOLD
			+ "%loc" + ChatColor.GREEN + " by " + ChatColor.GOLD + "%sender"),
	BL_ITEM_ALREADY("itemBlAlready", ChatColor.RED + "This item "
			+ ChatColor.GOLD + "%item " + ChatColor.RED
			+ "is already blacklisted"),
	BL_ITEM_PROBLEM("blItemSaveProb", ChatColor.GOLD
			+ "Can't save the blackList file"),
	...
	P_CLEARED_TARGET("powersClearedTarget", ChatColor.AQUA
			+ "All of your active powers have been removed by" + ChatColor.GOLD
			+ " %sender" + ChatColor.AQUA + "!"),
	P_CLEARED("powersCleared", ChatColor.GREEN
			+ "You have successfully removed all your powers!"),
	...
	NO_BANNED("noBannedPlayers", ChatColor.RED
			+ "There isn't any banned players."),
	...
	HEALED("heal", ChatColor.MAGIC + "aaaa" + ChatColor.GREEN + "healed"
	...
	BCAST("broadcast", ChatColor.YELLOW + "[BROADCAST]" + ChatColor.GOLD
			+ "%message"),
	UNKNOWN("unknown", "unknown"),
	IP_BROADCAST("ipBroadcast", ChatColor.GOLD + "%player " + ChatColor.RED
			+ "joined from the same IP" + ChatColor.GOLD + " as %player2"
			+ ChatColor.RED + ". The IP is: " + ChatColor.AQUA + "%ip"),
	GROUP_SPAWN_SET("groupSpawnSet", ChatColor.GREEN + "Spawn for group"
			+ ChatColor.GOLD + "%groupName" + ChatColor.GREEN + " set."),
	GROUP_SPAWN("groupSpawn", ChatColor.GREEN
			+ "You haven been successfully teleported to the"
			+ " spawn of group: " + ChatColor.GOLD + "%groupName"),
	NO_GROUP_SPAWN("noGroupSpawn", ChatColor.RED + "The group "
			+ ChatColor.AQUA + "%groupName" + ChatColor.RED
			+ " does not have a group spawn"),
	NO_SUCH_GROUP("noSuchGroup", ChatColor.RED + "A group with the name "
			+ ChatColor.AQUA + "%groupName" + ChatColor.RED
			+ " does not exist."),
	DEATH_MSG("deathMessage", ChatColor.AQUA + "%player " + ChatColor.YELLOW
			+ "%msg"),
	TELEPORT_SOON("tpSoonMsg", ChatColor.AQUA
			+ "You'll be teleported to your location in " + ChatColor.GREEN
			+ "%sec seconds"),
	AFK_KICK_BCAST("afkKickBroadcast", ChatColor.AQUA + "%player"
			+ ChatColor.GOLD
			+ " has been kicked out of the game due to being AFK too long."),
	NOT_A_BLOCK("notABlock", ChatColor.RED
			+ "You did not enter a valid block id!"),
	WARP_NO_PERM("noWarpPerm", ChatColor.RED
			+ "You do not have the permission to "
			+ "teleport to the following warp point: " + ChatColor.GOLD
			+ "%point"),
